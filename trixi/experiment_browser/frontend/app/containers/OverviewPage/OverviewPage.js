...
export default class OverviewPage extends React.PureComponent { // eslint-disable-line react/prefer-stateless-function
  ...
  // docu : https://alligator.io/react/axios-react/
  componentDidMount() {
    axios.get('http://localhost:5000/get_overview?dir=test_klaus')
      .then((res) => {
        console.log(res);
            ...
            {/* <div className="container-fluid">
              <div className="row text-left">
                ...
                <div className="col" style={buttonPaddingStyle}>
                  <div>
                    {baseInfo.ccols1.map((name) => <Button key={name} variant="outline-success">{name}</Button>)}
                  </div>
                  <div>
                    {baseInfo.ccols2.map((name) => <Button key={name} variant="outline-success">{name}</Button>)}
                  </div>
                </div>
              </div>
              <div className="row text-left">
                ...
                <div className="col" style={buttonPaddingStyle}>
                  {baseInfo.rcols.map((name) => <Button key={name} variant="outline-success">{name}</Button>)}
                </div>
              </div>
