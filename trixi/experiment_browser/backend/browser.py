...
from flask import Blueprint, Flask, abort, render_template, request, send_file, jsonify
...
def register_url_routes(app, base_dir):
    ...
    app.add_url_rule("/experiment_img", "experiment_img", lambda: experiment_img(base_dir), methods=["GET"])
    app.add_url_rule("/experiment_log", "experiment_log", lambda: experiment_log(base_dir), methods=["GET"])
    app.add_url_rule("/experiment_plot", "experiment_plot", lambda: experiment_plot(base_dir), methods=["GET"])
...
#     content["title"] = [exp.exp_name for exp in experiments] # not important
#     content["images"] = {"img_path": image_path, "imgs": images, "img_keys": image_keys}
#     content["plots"] = {"plots": plots}
#     content["config"] = {"exps": [exp.exp_name for exp in experiments], "configs": combi_config, "keys": config_keys,
#                          "diff_keys": diff_config_keys}
#     content["results"] = {"exps": exp_names, "results": combi_results, "keys": result_keys}
#     content["logs"] = {"logs_dict": logs_dict}
